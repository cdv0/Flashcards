...
if (globalSearchInput && setSearchInput) {
...
if (createTitleInput) {
  ...
  createSetConfirm.addEventListener("click", () => {
    ...
    const targetSection = sections[0]; // Unassigned by default
...
if (editTitleInput) {
  ...
  editDeleteBtn.addEventListener("click", () => {
    ...
    // IMPORTANT: close edit modal first, then open delete
    closeAllModals();
...
// ====== On first load: render & make sure modals are closed ======
window.addEventListener("DOMContentLoaded", () => {
  closeAllModals(); // ensure none are visible on refresh
  renderDashboard();
});
